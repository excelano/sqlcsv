...
func numericFixture() *Table {
	cols := []string{"ID", "Price", "Qty", "Discount", "Name"}
	schema := map[string]ColumnInfo{
		"ID":       {Name: "ID", Type: TypeInt},
		"Price":    {Name: "Price", Type: TypeInt},
		"Qty":      {Name: "Qty", Type: TypeInt},
		"Discount": {Name: "Discount", Type: TypeFloat},
		"Name":     {Name: "Name", Type: TypeString},
	}
	rows := []Row{
		{Cell{Int: 1}, Cell{Int: 10}, Cell{Int: 5}, Cell{Float: 0.1}, Cell{Str: "Widget"}},
		{Cell{Int: 2}, Cell{Int: 20}, Cell{Int: 0}, Cell{Float: 0.0}, Cell{Str: "Gizmo"}},
		{Cell{Int: 3}, Cell{Int: 30}, Cell{Int: 2}, Cell{Null: true}, Cell{Str: "Sprocket"}},
		{Cell{Int: 4}, Cell{Null: true}, Cell{Int: 4}, Cell{Float: 0.5}, Cell{Str: "Cog"}},
	}
	return &Table{
...
func TestEvalExprColumn(t *testing.T) {
	...
	if got.Cell.Int != 10 || got.Type != TypeInt {
...
func TestEvalExprLiteralInt(t *testing.T) {
	...
	if got.Type != TypeInt || got.Cell.Int != 42 {
...
func TestEvalExprLiteralFloat(t *testing.T) {
	...
	if got.Type != TypeFloat || got.Cell.Float != 3.14 {
...
func TestEvalExprArithmetic(t *testing.T) {
	...
	cases := []struct {
		...
		{"int + int", binE("+", colE("Price"), colE("Qty")), TypeInt, 15, 0},
		{"int - int", binE("-", colE("Price"), colE("Qty")), TypeInt, 5, 0},
		{"int * int", binE("*", colE("Price"), colE("Qty")), TypeInt, 50, 0},
		{"int / int promotes to float", binE("/", colE("Price"), colE("Qty")), TypeFloat, 0, 2.0},
		{"int + float promotes to float", binE("+", colE("Price"), colE("Discount")), TypeFloat, 0, 10.1},
		{"int * literal int", binE("*", colE("Qty"), litE(vnum("3"))), TypeInt, 15, 0},
		{"precedence: a + b * c", binE("+", colE("Price"), binE("*", colE("Qty"), litE(vnum("2")))), TypeInt, 20, 0},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantType == TypeInt && got.Cell.Int != tc.wantInt {
				t.Fatalf("int value: got %d, want %d", got.Cell.Int, tc.wantInt)
			}
			if tc.wantType == TypeFloat && got.Cell.Float != tc.wantFlt {
...
func TestEvalExprNullPropagates(t *testing.T) {
	...
	// Row 2: Discount=NULL. Price * Discount should be NULL.
	row := tbl.Rows[2]
	...
	// Row 3: Price=NULL. NULL + 1 should be NULL.
	row = tbl.Rows[3]
...
func TestMatchesArithmeticNullExcludes(t *testing.T) {
	...
	// Row 1 has Qty=0, so Price/Qty = NULL → row excluded.
	// Row 3 has Price=NULL → row excluded.
